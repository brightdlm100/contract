...
import com.dudu.mongo.domin.UpdateRecord_ContractOverview_GuiYang;
import com.dudu.mongo.domin.UpdateRecord_ContractOverview_GuiYang.UpdateRecord_Contract;
import com.dudu.mongo.domin.贵阳合同;
import com.dudu.mongo.domin.贵阳合同.历史记录;
import com.dudu.mongo.domin.贵阳合同.贷款信息;
...
public class ContractOfGuiYang {
	static  int retry=1000000000;
	static int totalPage=-1;
	static int totalCount=-1;
    static String url="http://crm.yango.com.cn:8060/_grid/griddata.aspx?xml=/Slxt/XSXC/Contract_Grid1.xml&gridId=appGrid&sortCol=QSDate&sortDir=descend&vscrollmode=0&multiSelect=1&selectByCheckBox=0&filter=&processNullFilter=1&customFilter=(2=2)&customFilter2=&dependencySQLFilter=&location=&pageNum=1&pageSize=10&showPageCount=1&appName=Default&application=&cp=";   
    //static String url=     "http://crm.yango.com.cn:8060/_grid/griddata.aspx?xml=%2fSlxt%2fXSXC%2fContract_Grid1.xml&gridId=appGrid&sortCol=QSDate&sortDir=descend&vscrollmode=0&multiSelect=1&selectByCheckBox=0&filter=%3centity+name%3d%22es_Contract%22+primarykey%3d%22ContractGUID%22%3e%3cfilter+type%3d%22and%22%3e%3cfilter+type%3d%22and%22%2f%3e%3cfilter+type%3d%22and%22%3e%3ccondition+operator%3d%22in%22+attribute%3d%22ProjGUID%22+value%3d%22589d06c6-073e-47e9-a9b3-40e50744f2f3%22%2f%3e%3c%2ffilter%3e%0d%0a%3c%2ffilter%3e%3c%2fentity%3e&processNullFilter=1&customFilter=(2%3d2)&customFilter2=&dependencySQLFilter=&location=&pageNum=1&pageSize=10&showPageCount=1&appName=Default&application=&cp=";
    static HashMap<String, String> cookies = null;
	public static void main(String[] args) {
		 
	     int  state=login(3, "xufang10", "Xufang198800");
	     if(state<0) {
	    	 return;
	     }
		compareAll(args);
	...
	public static int login(int retry,String username,String password)  {
		...
		try {
			   ...
			   // http://crm.yango.com.cn:8060/Slxt/XSXC/Contract_Grid.aspx?xml=/Slxt/XSXC/Contract_Grid1.xml&filter=<entity name="es_Contract" primarykey="ContractGUID"><filter type="and"><filter type="and"/><filter type="and"><condition operator="in" attribute="ProjGUID" value="589d06c6-073e-47e9-a9b3-40e50744f2f3"/></filter></filter></entity>
			    ...
			    filter="<entity name=\"es_Contract\" primarykey=\"ContractGUID\"><filter type=\"and\"><filter type=\"and\"/><filter type=\"and\"><condition operator=\"in\" attribute=\"ProjGUID\" value=\"589d06c6-073e-47e9-a9b3-40e50744f2f3\"/></filter>\r\n" + 
			    		"</filter></entity>";
	...
	public static UpdateRecord_ContractOverview_GuiYang compareAll(String[] args) {
		...
		System.out.println("网络上共有记录"+totalCount+"条,共"+totalPage+"页.");
		 ...
		 final UpdateRecord_ContractOverview_GuiYang overview=new UpdateRecord_ContractOverview_GuiYang();
	    	       ...
	    	       public void run() {                       //去掉注释使用多线程
	    	           ...
	    	           if(doc==null) {
	    	        	   ...
	    	        	   for (Element tr : trs) {
	    	        		   ...
	    	        		   贵阳合同 exitsted=mongoTemplate.findOne(new Query(Criteria.where("房间").is(contract.房间)), 贵阳合同.class);
	    	        		   ...
	    	        		   getContractDetail(j,overview,contract,detailurl);
	    	        		    	  ...
	    	        		    	  e.printStackTrace();
					                  System.err.println("解析贵阳合同"+tr.getElementsByTag("td").get(1).text()+"的详细信息失败!");
					                  overview.failureLog.add("解析贵阳合同"+tr.getElementsByTag("td").get(1).text()+"的详细信息失败!");
								}
	    	        		    if(contract!=null) {
	    	        		    	try {
	    	        		            compareWithDB(j,mongoTemplate,contract,overview);
	    	        		    	}catch (Exception e) {
...
public static void getContractDetail(int j,UpdateRecord_ContractOverview_GuiYang overview, 贵阳合同 contract, String detailurl) throws Exception {
	    try {
	    //System.out.println("--------------------正在获取第"+j+"页,"+contract.客户名称+":"+contract.房间+"的贵阳合同详情......");
		Document doc=HttpclientUtil.requestUrl(retry,"get",detailurl, cookies, null);
		if(doc==null) {
			System.err.println("第"+j+"页,"+contract.客户名称+":"+contract.房间+"的贵阳合同详情获取失败!");
			overview.failureLog.add("第"+j+"页,"+contract.客户名称+":"+contract.房间+"的贵阳合同详情获取失败!");
		}else {
			processDetail(contract,doc,overview.failureLog);
		}
	    }catch (Exception e) {
			throw new Exception("获取贵阳合同详情失败");
		}
	}
	...
	private static void compareWithDB(int j, MongoTemplate mongoTemplate, 贵阳合同 contract, UpdateRecord_ContractOverview_GuiYang overview) throws Exception {
       try { 
    	   贵阳合同 existContract=mongoTemplate.findOne(new Query(Criteria.where("房间").is(contract.房间)), 贵阳合同.class);
	...
	private static void processDetail(贵阳合同 contract, Document doc, ConcurrentLinkedQueue<String> failureLog) throws Exception {
		 try {
				/*
		        基本信息
		    */
	...
	for (Element e : rurls) {
	 ...
	 contract.房间信息=rInfoQueue;
			...
			throw new Exception("获取贵阳合同细节失败!");
		}
	...
	public static void getContractTotal() {
		try {
	...
	public static UpdateRecord_ContractOverview_GuiYang PagesRetry(ArrayList<String>pages,String[] args) {
		 ApplicationContext applicationContext= SpringApplication.run(Application.class,args);
		 ContractService contractService=(ContractService) applicationContext.getBean("contractService");    
		 MongoTemplate mongoTemplate=(MongoTemplate) applicationContext.getBean("mongoTemplate");
