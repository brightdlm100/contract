...
public class ContractTaskService {
    ...
    public void beforeInit() {
    	contractService=contractService1;
    ...
    public void ContractTask(){
		...
		try {
				...
				for(int i=1;i<=retry;i++) {
					...
					if(res==null) {
						logger.error("第"+i+"次访问登陆页面失败！"+urlLogin);  
					}else {
						// 获取返回的cookie
				...
				if(cookies==null) {
					...
					logger.error("无法请求登陆cookies，操作无法继续进行！");
					return -1;
				}
			   ...
			   // System.out.println(JSON.toJSONString(cookies));
			    ...
			    if(doc2==null) {
			    	...
			    	logger.error("无法获取TempValue,操作无法继续!");
			    	return -1; 
			    }
			    String filter=doc2.getElementById("txtFilter").attr("value");
			    basicurl=url.replace("filter=", "filter="+URLEncoder.encode(filter,"utf-8"));
			...
			logger.error("登陆过程中出错！");
			return -1;
		}finally {
			long endTime=System.currentTimeMillis();
			System.out.println("登陆操作结束，用时:"+StringUtil.formatDuring(endTime-startTime));
			logger.info("登陆操作结束，用时:"+StringUtil.formatDuring(endTime-startTime));
		}
	}
	
	public static UpdateRecord_ContractOverview compareAll_Single() {
		 final long startTime1=System.currentTimeMillis();                                               //获取开始时间
		getContractTotal();
		//totalPage=40;
		logger.info("网络上共有记录"+totalCount+"条,共"+totalPage+"页.");
		System.out.println("网络上共有记录"+totalCount+"条,共"+totalPage+"页.");
		int dbSize=mongoTemplate.find(new Query(),合同.class).size();
		String dbmsg="数据库上共有记录:"+dbSize+"条。";
		logger.info(dbmsg);
		System.out.println(dbmsg);
		//totalPage=40;
	    //System.out.println(totalPage);
		 if(totalPage<=0) {
			 String cannotGetTotalPage="无法获取到总页数,操作无法继续进行...";
	  	   logger.error(cannotGetTotalPage);
	  	   System.err.println(cannotGetTotalPage);
	       return null;
	     }
		 ...
		 for(int i=1;i<=totalPage;i++) {
			 int thisPageChanged=0;
			 int CurrentIgnored=overview.totalignore.get();
			 int CurrentInserted=overview.totalinsert.get();
			 int CurrentUpdated=overview.totalupdate.get();
			final int j=i;  
			//Runnable runnable = new Runnable() {          //去掉注释使用多线程
	  	       ...
	  	       //public void run() {                       //去掉注释使用多线程
	  	    	   final long startTime=System.currentTimeMillis(); 
	  	            ...
	  	            String listurl=basicurl.replace("pageNum=1", "pageNum="+j);
	  	            //System.out.println(listurl);
	  	            Document doc=HttpclientUtil.requestUrl(retry,"get", listurl, cookies, null);
	  	            //System.out.println(doc);
	  	           if(doc==null) {
	  	        	    logger.error("-----请求第"+j+"页失败,已放弃:"+listurl);
	  	        	   ...
	  	        	   for (Element tr : trs) {
	  	        		  ...
	  	        		  contract.客户名称=tds.get(2).text();
	  	        		   ...
	  	        		   if(exitsted!=null&&exitsted.联系电话!=null&&exitsted.客户信息!=null&&exitsted.房间信息!=null&&exitsted.客户名称.equals(contract.客户名称)) {
	  	        			   overview.totalignore.incrementAndGet();
	  	        			  // System.out.println("~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~记录已完整存在,跳过.当前已跳过"+overview.totalignore+"条记录。");
	  	        			   continue;
	  	        		   }
	  	        		   ...
	  	        		   getContractDetail(j,overview,contract,detailurl);
	  	        		    	  ...
	  	        		    	  e.printStackTrace();
					                 // System.err.println("解析合同"+tr.getElementsByTag("td").get(1).text()+"的详细信息失败!");
	  	        		    	     logger.error("解析合同"+tr.getElementsByTag("td").get(1).text()+"的详细信息失败!");
	  	        		    	...
	  	        		    	try {
	  	        		            compareWithDB(j,contractService,mongoTemplate,contract,overview);
										...
										//System.err.println(contract.房间+"与数据库比对过程中出错!");
	  	        		    		    logger.error(contract.房间+"与数据库比对过程中出错!");
	  	        		...
	  	        		logger.error("解析第"+j+"页数据出错!");
	  	        		 System.err.println("解析第"+j+"页数据出错!");
			     		 overview.failureLog.add("解析第"+j+"页数据出错!");
			     		}finally {
			     			 final long endTime=System.currentTimeMillis(); 
			     			 thisPageChanged=overview.totalinsert.get()-CurrentInserted+overview.totalupdate.get()-CurrentUpdated;
			     			   //System.out.println("当前第"+j+"页,插入:"+overview.totalinsert+",更新:"+overview.totalupdate+",跳过:"+overview.totalignore+"，用时:"+StringUtil.formatDuring(endTime-startTime)+"，总用时"+StringUtil.formatDuring(endTime-startTime1));
			     			 System.out.println("当前第"+j+"页,本页插入:"+(overview.totalinsert.get()-CurrentInserted)+",本页更新:"+(overview.totalupdate.get()-CurrentUpdated)+",本页跳过:"+(overview.totalignore.get()-CurrentIgnored)+"，本页用时:"+StringUtil.formatDuring(endTime-startTime)+"总用时"+StringUtil.formatDuring(endTime-startTime1));
			     			 if(thisPageChanged==0) {
			     				 System.out.println("由于本页无任何更新或插入，故后续页面将不进行处理。");
			     				 logger.info("由于本页无任何更新或插入，故后续页面将不进行处理。");
			     				 break;
			     			 }
			     			 // latch.countDown();
		    		        //   semaphore.release();
		 ...
		 System.out.println("本次插入："+overview.totalinsert+",更新："+overview.totalupdate+"，跳过："+overview.totalignore+".");
	...
	public static UpdateRecord_ContractOverview compareAll_Multi() {
		 final long startTime1=System.currentTimeMillis();                                               //获取开始时间
		getContractTotal();
		
		System.out.println("网络上共有记录"+totalCount+"条,共"+totalPage+"页.");
		int dbSize=mongoTemplate.find(new Query(),合同.class).size();
		System.out.println("数据库上共有记录:"+dbSize+"条。");
		totalPage=40;
	    //System.out.println(totalPage);
		 if(totalPage<=0) {
	  	        		  ...
	  	        		  // break;
	  	        		   ...
	  	        		   System.err.println("解析第"+j+"页数据出错!");
			     			  ...
			     			  final long endTime=System.currentTimeMillis(); 
			     			   ...
			     			   //System.out.println("当前第"+j+"页,插入:"+overview.totalinsert+",更新:"+overview.totalupdate+",跳过:"+overview.totalignore+"，用时:"+StringUtil.formatDuring(endTime-startTime)+"，总用时"+StringUtil.formatDuring(endTime-startTime1));
			     			 System.out.println("当前第"+j+"页,插入:"+overview.totalinsert+",更新:"+overview.totalupdate+",跳过:"+overview.totalignore+"，总用时"+StringUtil.formatDuring(endTime-startTime1));
			     			  latch.countDown();
		    		           semaphore.release();
			     		}
	  	       }                                             																							//去掉注释使用多线程
	...
	private static void compareWithDB(int j, ContractService contractService, MongoTemplate mongoTemplate, 合同 contract, UpdateRecord_ContractOverview overview) throws Exception {
	 try { 
		   ...
		   }else if(existContract.联系电话==null||existContract.客户信息==null||existContract.房间信息==null||!(existContract.客户名称.equals(contract.客户名称))) {
	...
	public static void getContractTotal() {
	  ...
	  Document  doc=HttpclientUtil.requestUrl(retry,"get", basicurl, cookies, data);
	  //System.out.println(doc);
